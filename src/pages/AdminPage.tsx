...
const AdminPage: React.FC<Props> = ({ onBack, branding }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const url = `${API}?action=get_beitraege&kundenId=${KUNDEN_ID}`;
  ...
  const handleSave = async () => {
    ...
    try {
      const params = new URLSearchParams({
        action: 'add_beitrag', kundenId: KUNDEN_ID,
        vereinName: branding.vereinName, titel: titel.trim(),
        text: text.trim(), bildUrl: bildUrl.trim(),
        videoUrl: videoUrl.trim(),
        datum: new Date().toLocaleDateString('de-DE'), kategorie,
      });
  ...
  const inputStyle: React.CSSProperties = {
    ...
    fontFamily: 'inherit', background: 'white', boxSizing: 'border-box', color: '#111',
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', backgroundColor: '#f0f0f0', padding: 14 }}>
        {!showForm && (
          <button onClick={() => setShowForm(true)} style={{ width: '100%', padding: 14, borderRadius: 10, backgroundColor: farbe, border: 'none', color: 'white', fontWeight: 700, fontSize: 16, cursor: 'pointer', marginBottom: 16 }}>
        ...
        {showForm && (
          <div style={{ background: 'white', borderRadius: 14, padding: 16, marginBottom: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.10)' }}>
            ...
            {success && <p style={{ color: '#34a853' }}>{success}</p>}
